// Cosmic Orb Background — Vanilla JS port of Originkit's Cosmic Orb
// Renders a full-bleed WebGL cosmic orb behind the hero-particle section.
...
void main() {
  ...
  gl_FragColor = vec4(shade(p), 1.0);
    ...
    function compileShader(gl, type, src) {
        ...
        console.error("[CosmicOrbBg] shader compile failed:", gl.getShaderInfoLog(sh));
        gl.deleteShader(sh);
        return null;
    }
    ...
    function initCosmicOrbBackground() {
        const heroSection = document.getElementById("home");
        if (!heroSection) return;

        // Create the canvas element
        const canvas = document.createElement("canvas");
        canvas.id = "cosmicOrbCanvas";
        canvas.style.cssText =
            "position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:100%;height:100%;max-width:1113px;max-height:1113px;aspect-ratio:1/1;z-index:2;pointer-events:none;";

        // Insert canvas as child in hero section
        heroSection.appendChild(canvas);

        // Build config from preset props
        const palette = DEFAULTS.palette;
        const cfg = {
            bg: toRGB(DEFAULTS.background),
            anchor: toRGB(palette.anchor),
            c0: toRGB(palette.colorA),
            c1: toRGB(palette.colorB),
            c2: toRGB(palette.colorC),
            speed: Math.max(0, DEFAULTS.speed) / 50,
            spin: Math.max(0, DEFAULTS.spin) / 50,
            audio: 0,
            arch: DEFAULTS.archetype === "auto" ? -1 : ARCHETYPES.indexOf(DEFAULTS.archetype),
            lens: DEFAULTS.lens ? (Math.max(0, DEFAULTS.lensAmount) / 100) * 0.2 : 0,
            phase: PHASE,
        };
        ...
        if (!gl) {
            console.warn("[CosmicOrbBg] WebGL not available");
            return;
        }
        ...
        function init() {
            ...
            if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
                console.error("[CosmicOrbBg] link failed:", gl.getProgramInfoLog(p));
                return;
            }
            prog = p;
            u = {};
        ...
        // Resize handler — fill the hero section
        function resize() {
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initCosmicOrbBackground);
    } else {
        initCosmicOrbBackground();
    }
})();
